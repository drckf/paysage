...
class TAP_rbm(model.Model):
    ...
    def helmholtz_free_energy_old(self, seed=None, init_lr=0.1, tol=1e-4, max_iters=50, terms=2, method='gd'):
        """
    ...
    def grad_helmholtz_free_energy_old(self, num_r, num_p):
        """
        Compute the gradient of the Helmholtz free engergy of the model according to the TAP
        expansion around infinite temperature.
        ...
        # compute minimizing magnetizations from random initializations
        for s in range(num_r):
            (m,EMF) = self.helmholtz_free_energy_old(None,
                                                 self.init_lr_EMF,
                                                 self.tolerance_EMF,
                                                 self.max_iters_EMF,
                                                 self.terms)
