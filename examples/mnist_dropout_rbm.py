...
def run(paysage_path=None, num_epochs=10, show_plot=False):
    ...
    vis_layer = layers.BernoulliLayer(data.ncols, dropout_p=0.0)
    hid_layer = layers.BernoulliLayer(num_hidden_units, dropout_p=0.5)

    rbm = model.Model([vis_layer, hid_layer])
    rbm.weights[0].add_penalty({'matrix': pen.l2_penalty(.001)})
